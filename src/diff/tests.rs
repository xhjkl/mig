...
fn generated_rust_is_flagged_and_forced_through_line_syntax() {
    ...
    let marker_added = diff_file("alpha.rs", after, before).expect("plain diff cannot fail");
...
fn malformed_rust_falls_back_without_hiding_source_changes() {
    let before = "fn alpha(value: u32 {}\n";
    let after = "fn alpha(value: u64 {}\n";
    ...
    assert!(
        rows.iter().any(|row| {
            matches!(row, ReviewRow::Removed(line) if line_text(line).contains("u32"))
        })
    );
    assert!(
        rows.iter().any(|row| {
            matches!(row, ReviewRow::Added(line) if line_text(line).contains("u64"))
        })
    );
}
