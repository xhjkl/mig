...
fn real_git_scan_covers_the_net_recursive_worktree() {
    ...
    assert!(
        diffs
            .iter()
            .all(|review| { matches!(review, ReviewEntry::Diff(diff) if !diff.hunks.is_empty()) })
    );
...
fn review_compares_pinned_head_directly_to_the_worktree() {
    ...
    let ReviewEntry::Diff(diff) = &reviews[0] else {
...
fn exact_limit_is_still_diffed() {
    ...
    assert!(matches!(
        reviews.as_slice(),
        [ReviewEntry::Diff(diff)] if diff.path == "exact.txt" && !diff.hunks.is_empty()
    ));
}
